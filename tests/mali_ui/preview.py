...
def preview(w,h,labels,index,name):
 ...
 def icon(label,x,y,size,c):
  ...
  elif label in ['FERRAMENTAS','KEY GAUGE','PIXEL PAINT']:
 ...
 details={'FERRAMENTAS':'Utilitarios Mali','KEY GAUGE':'Perfis geometricos','COUNTER':'Testes de resiliencia','RADIO':'RF / IR / NFC'}
 for k in range(-2,3):
  ...
  if selected:
   detail=details.get(label,'Clique para abrir');n=(cardw-inset-44)//6;detail=detail if len(detail)<=n else detail[:n-3]+'...';text(detail,cardx+inset+37,y+hh//2+9,1,'TEXT_SECONDARY')
 ...
 d.rectangle((0,h-16,w,h),fill=colors['BACKGROUND']);text('Girar / Clique / Seg: voltar' if portrait else 'Girar: escolher Clique: abrir Segurar: voltar',6,h-12,1,'TEXT_SECONDARY')
 im.save(f'tests/mali_ui/gear-{name.lower()}-{w}x{h}.png')
 return im
root=['REDE','RADIO','FERRAMENTAS','COUNTER','ARQUIVOS','SISTEMA'];tools=['PIXEL PAINT','D20','COUNTER','KEY GAUGE','UTILITARIOS']
images=[preview(320,170,root,2,'OS'),preview(170,320,root,2,'OS'),preview(170,320,tools,3,'FERRAMENTAS')]
